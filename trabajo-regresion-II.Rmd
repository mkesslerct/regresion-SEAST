...
```{r}
## Completar aqui
GradosTeleco <- read.table(file = "data/notamediagradosETSIT-completo.csv", header=TRUE, sep=";", dec=".", stringsAsFactors = FALSE)
```
...
resumenerrores <- errores %>%
  ...
  summarise(
    er_1=mean(errores^2, na.rm = TRUE), 
    er_2= (sum(abs(errores)<0.25, na.rm = TRUE) / n()) * 100,
    er_3 =(sum(abs(errores)<0.1,  na.rm = TRUE) / n()) * 100)
resumenerrores
...
errores.predict.PAU.primercurso <- GradosTeleco.test$notamedia -  test.predict.PAU.primeroscursos
```
...
errores <- rbind(errores,
                 data.frame(caracteristicas = "PAU y primer curso",
                            errores = errores.predict.PAU.primercurso))
```
...
```{r}
### Completar aquí, usando ggplot con geom_density,
ggplot(data = errores, aes(x = errores, group=caracteristicas, fill=caracteristicas)) + geom_density(alpha = 0.5)
```
...
training.lm.completo <- lm(data=GradosTeleco.training,notamedia ~ . -ALU_PLAN_ID - ALU_EXP_ID)
```
### Predicción de la nota media para el conjunto test
...
GradosTeleco.test$notamediaTest <- test.predict.completo
ggplot(data = GradosTeleco.test, aes(x = PAU_CALIFICACION_NOTA_MATERIA, y= notamedia)) + geom_point() +
  geom_point(aes(y = notamediaTest), colour="red")
