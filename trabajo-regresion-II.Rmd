...
plot(training.lm.PAU, which = 1)
plot(training.lm.PAU, which = 2)
...
resumenerrores <- errores %>% group_by(caracteristicas) %>%
  summarise(
    Error1= mean(errores^2, na.rm = TRUE),
    Error2 = 100*(sum(abs(errores)<0.25, na.rm = TRUE)/length(errores)),
    Error3 = 100*(sum(abs(errores)<0.1, na.rm = TRUE)/length(caracteristicas))
  )
```
...
test.predict.PAU.primercurso <- predict(training.lm.PAU.primercurso, GradosTeleco.test)
```
...
errores.predict.PAU.primercurso <-  GradosTeleco.test$notamedia - test.predict.PAU.primercurso
...
> Hay menos densidad de error conforme más datos tomamos por lo que la respueta es sí.
