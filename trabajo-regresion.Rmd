...
```{r}
## Completar aqui"
GradosTelecoSimple <- read.table("./data/notamediagradosETSIT-simple.csv", header=TRUE, sep = ",")
```
...
p <- ggplot(data=GradosTelecoSimple, aes(y = notamedia, x = PAU_CALIFICACION_NOTA_MATERIA)) + geom_point()
...
ggplot(data=GradosTelecoSimple, aes(y = notamedia, x = PAU_CALIFICACION_NOTA_MATERIA_DECIL)) + geom_point()
...
vectorgrad=(2/length(y))* t (x) %*% (x %*% theta - y)
return(vectorgrad)}
...
## Completar aquí
iter<-i:maxiter
ggplot(monitor, aes(y = J, x = iter)) + geom_line()
```
...
iter <- 50:maxiter
ggplot(monitor2, aes(y= J, x = iter)) + geom_line(color="red")
```
...
ggplot(data = monitor_filter, aes(x = iter, y = J, color=alpha)) + geom_line()
```
