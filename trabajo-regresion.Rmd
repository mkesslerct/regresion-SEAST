...
```{r}
## Completar aqui"
ggplot(data = GradosTelecoSimple, aes(y = notamedia, x = PAU_CALIFICACION_NOTA_MATERIA)) + geom_point()
```
...
## Completar aquí
iter<-1:maxiter
ggplot(monitor, aes(y = J, x = iter)) + geom_line(color="red")
```
...
for (alpha in c(0.001,0.003,0.01))
{
  theta=thetainicial
...
monitor3<-monitor %>% 
  filter(iter>50)
iter<-50:maxiter
ggplot(monitor3, aes(y = J, x = iter)) + geom_line()
