...
monitor_filter <- monitor %>%
  filter(between(iter, 50, 3000))

ggplot(data = monitor_filter, aes(x = iter, y = J)) + geom_line()
...
>Decrece tan lentamente porque en las últimas iteracciones el gradiente es muy próximo a 0, y al ser alpha tan pequeño la diferencia entre cada iteracción es prácticamente inexistente. Habría que aumentar alpha
...
for(alpha_i in alpha)
{
  ...
  for (i in 1:maxiter)
  {
    theta <- theta - alpha_i * gradJ(theta)
    monitor[index,]=c(i, theta, J(theta), gradJ(theta), alpha_i)
    index = index + 1
  }
}
```
...
ggplot(data = monitor_filter, aes(x = iter, y = J, colour=alpha)) + geom_line()
```
...
while(i <= maxiter && deltaJ > 10^(-6))
{
   Jprev <- J(theta)
   theta <- theta - alpha * gradJ(theta)
   deltaJ <- abs(J(theta) - Jprev)
   monitor[i,]=c(i, theta, J(theta), gradJ(theta), alpha)
   i=i+1
}
```
...
```{r}
## completar aquí
r <- residuals(notamediagrados.lm)
mean(r^2)
```
