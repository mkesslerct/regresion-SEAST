...
```{r}
## completar aquí...
monitor %>% filter(between(iter,50,3000)) %>%
...
Como estamos cerca del minímo local y al tratrarse de un método numérico, la pendiente es pequeña y por tanto disminuye con lentitud.
...
monitor %>% filter(iter > 50) %>%
  ggplot(aes(x = iter, y = J, colour = alpha)) + geom_line()
```
...
```
Representar gráficamente la evolución del coste para estas 30 iteraciones...
...
i <- 0;
theta=thetainicial;
while(i < maxiter && norm(gradJ(theta)) > 10^(-6)){
  theta <- theta - alpha * gradJ(theta)
  monitor[i,]=c(i,theta,J(theta),gradJ(theta), alpha)
  i <- i + 1
}
...
(Utilizo i-1 ya que no se alcanza el máximo numero de iteraciones, se cumple antes la condición de error, por tanto la última iteración es i-1)
Estimación de theta: `r monitor[i-1,"theta0"]` y `r monitor[i-1,"theta1"]`

Coste mínimo alcanzado: `r monitor[i-1,"J"]`
...
Nos da un valor muy similar a nuestra estimación anterior, por lo que decidimos que es un valor correcto.
...
```{r}
## completar aquí
mean(residuals(notamediagrados.lm)^2)
```
