...
```{r}
## Completar aqui"
GradosTelecoSimple <- read.table(file = "data/notamediagradosETSIT-simple.csv", header = TRUE, sep = ",", dec = ".", stringsAsFactors = FALSE)
```
...
```{r}
## Completar aqui"
ggplot(data = GradosTelecoSimple, aes(x=PAU_CALIFICACION_NOTA_MATERIA, y=notamedia)) + geom_point()
```
...
```{r}
# Completar aquí
ggplot(data = GradosTelecoSimple, aes(x=PAU_CALIFICACION_NOTA_MATERIA_DECIL, y=notamedia)) + geom_point()
...
notamedia <- as.matrix(cbind(1,GradosTelecoSimple$PAU_CALIFICACION_NOTA_MATERIA))
```
...
J <- function(theta)
  ...
  coste <- (1/length(y)) * t(y-notamedia%*%theta) %*% (y-notamedia%*%theta)
...
gradJ <- function(theta)
  ...
  gradiente <- (2/length(y))*t(notamedia)%*%(notamedia%*%theta-y)
...
```{r}
## Completar aquí
ggplot(data = monitor, aes(x=iter, y=J)) + geom_line()
```
...
```{r}
## completar aquí...
monitor %>% filter(between(iter,50,3000)) %>% ggplot(aes(x=iter, y=J)) + geom_line()
```
...
> Conforme nos acercamos al mínimo local el descenso de gradiente es más lento (da pasos más pequeños).
...
for (i in 1:maxiter)
  {
    theta <- theta - alpha * gradJ(theta)
    monitor[i+maxiter*(j-1),]=c(i, theta, J(theta), gradJ(theta), alpha)
...
monitor%>% filter(iter>50)%>%
  ggplot(aes(x=iter, y=J, color=factor(alpha))) + geom_line()
```
...
```
Representar graficamente la evolución del coste para estas 50 iteraciones...

```{r}
## Completar aquí: ggplot evolución del coste en función de iter...
monitor %>% filter(iter>0)%>%
  ggplot(aes(x=iter, y=J)) +geom_line()
```
...
Comentarios: Nuestra estimación final era -0.0108 y 0.2127 utilizando el descenso de gradiente. Utilizando la función lm obtenemos -0.01076 y 0.21267. It works!
...
```{r}
## completar aquí
names(notamediagrados.lm)
sum(residuals(notamediagrados.lm)^2)/length(residuals(notamediagrados.lm))
```
